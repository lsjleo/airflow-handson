...
c.PAMAuthenticator.admin_groups = {'admin'}
...
# class CustomGoogleAuthenticator(LocalGoogleOAuthenticator):
#     def normalize_username(self, username):
#         return username.split('@')[0].lower()


# ## c.JupyterHub.authenticator_class = CustomGoogleAuthenticator # Autenticação sem SSO -- MANTER COMENTADO
...
# c.OAuthenticator.add_user_cmd = ['adduser', '-q', '--gecos', '""', '--disabled-password', '--force-badname']
...
# c.LocalGoogleOAuthenticator.open_sessions = False
...
c.JupyterHub.db_url = os.getenv('JHUB_DB')
...
#     <input name="snowflake_key" val="snowflake_key"></input>
#     <input name="aws_key" val="aws_key"></input>
# '''
...
c.Authenticator.allow_all_users_by_default = True
...
# c.GitHubConfig.access_token = os.getenv('GITHUB_TOKEN')

"""
JupyterHub config allowing users to specify environment variables and notebook-server args
"""
# import shlex
import os
import configparser
